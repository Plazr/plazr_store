module PlazrStore
  class Admin::ProductPropertiesController < ApplicationController
    ...
    def create
      ...
      if Property.exists?(:id_name => params[:property][:id_name]) && 
      ...
      # also checks if the user didn't want to enter a new product_property and if it is the case
      # just get all the product_properties
      elsif (params[:property][:id_name].blank? && 
          params[:property][:product_properties_attributes]['0'][:value].blank?) 
        @product_properties = @product.product_properties
